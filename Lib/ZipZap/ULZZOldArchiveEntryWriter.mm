...
@implementation ULZZOldArchiveEntryWriter
{
...
- (uint32_t)offsetToLocalFileEnd
{
	if (_localFile)
		...
		return ((const ULZZCentralFileHeader*)_centralFileHeader.bytes)->relativeOffsetOfLocalHeader + _localFileLength;
}
								...
								error:(out NSError**)error
{
	if (_localFile)
	{
		...
		((ULZZCentralFileHeader*)((NSMutableData*)_centralFileHeader).mutableBytes)->relativeOffsetOfLocalHeader = [channelOutput offset] + initialSkip;

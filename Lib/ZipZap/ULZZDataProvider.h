...
public:
	static CGDataProviderRef create(NSInputStream*(^makeStream)())
	{
		...
		ULZZDataProvider* dataProvider = new((ULZZDataProvider*)malloc(sizeof(ULZZDataProvider))) ULZZDataProvider(makeStream);
		...
		CGDataProviderSequentialCallbacks dataProviderCallbacks =
		{
			...
			{
				return reinterpret_cast<ULZZDataProvider*>(info)->getBytes(buffer, count);
			},
			[](void* info, off_t count) -> off_t
			{
				return reinterpret_cast<ULZZDataProvider*>(info)->skipForwardBytes(count);
			},
			[](void* info)
			{
				reinterpret_cast<ULZZDataProvider*>(info)->rewind();
			},
			[](void* info)
			{
				reinterpret_cast<ULZZDataProvider*>(info)->~ULZZDataProvider();
				free(info);
			}
		};
...
private:
	ULZZDataProvider(NSInputStream*(^makeStream)()): _makeStream(makeStream), _stream(nil)
	{
